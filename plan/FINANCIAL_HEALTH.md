...
**`/net-worth` ganhou o mesmo passivo (M-85, 2026-08-25).** Até aqui só `/health` enxergava dívida de parcelamento em conta comum (`getDebtBreakdown` acima) — `/net-worth` só reconhecia passivo em contas `CREDIT`/`LOAN`, nunca em transações. Na prática isso levava o usuário a criar uma conta `LOAN` manual duplicando uma dívida que já estava lançada como série de parcelamento, só para ela aparecer no Patrimônio — e a mesma dívida acabava contada duas vezes em `/health` (a série real + o `LOAN` manual, com valores divergentes por arredondamento). Corrigido reaproveitando `getDebtBreakdown` também em `/net-worth`: mesmo motor, mesma definição do que conta como dívida, sem lógica nova. Reforça a leitura do parágrafo do `HE-15` acima — `LOAN` é para dívida **sem** lançamento correspondente (ex.: dinheiro emprestado informalmente por um familiar); se a dívida já é uma série de parcelamento, ela não precisa (e não deve) também virar um `LOAN`. Ver `plan/BACKLOG.md` M-85.

**`/net-worth` — cartão de crédito também passa a contar o total parcelado, não só a fatura atual (M-86, 2026-08-25).** O M-85 só cobriu parcelamento em conta comum; o passivo de cartão em `/net-worth` continuava escopado a `getTotalCreditLiability`/`getOpenCreditBalance` (fatura atual em aberto), a mesma limitação que o `HE-15` original resolveu para `/health`. Pedido do usuário: ressignificar "Total Comprometido" do cartão em `/net-worth` para a mesma definição de `getDebtBreakdown` (`kind: 'card'`) que `/health` já usa — soma de todas as parcelas em aberto, não só a do mês corrente. Cabeçalho do card "Cartões de Crédito" e os stat cards Passivos/Patrimônio Líquido passam a refletir essa definição (decisão explícita do usuário — não só a coluna da linha). `getTotalCreditLiability` ficou sem nenhum uso em produção depois disso e foi removido de `lib/utils.ts` (junto de seus testes, redundantes com os de `getOpenCreditBalance`, que segue intocada e é a base do limite disponível e da "Fatura atual"). Card "Parcelamentos" renomeado para "Parcelamentos em Contas" e reordenado (Parcela Mensal → Total Comprometido), para ler na mesma ordem "do mês → do todo" do card de cartões (Fatura Atual → Total Comprometido). Ver `plan/BACKLOG.md` M-86.
...
**Comprometimento fixo recorrente (M-63, parte Saúde Financeira).** Até aqui a tela só enxergava dívida (`tx.installment`/`LoanMetadata`) — uma assinatura ou conta recorrente (`tx.recurrence`, ex.: Netflix, aluguel) lançada como série comum não aparecia em nenhuma métrica, mesmo sendo um comprometimento mensal real e previsível. `getRecurringCommitment(transactions, referenceDate)` (`lib/utils.ts`, ao lado de `projectRecurringOccurrences`) agrupa por `recurrence.parentId`, filtra só `EXPENSE`, considera ativa toda série sem `endDate` ou com `endDate` ainda no futuro, e soma o valor do template (última ocorrência) normalizado para equivalente mensal pela frequência (`weekly` ×52/12, `biweekly` ×26/12). **Decisão deliberada:** vira uma linha informativa própria no Card 1 (abaixo do compromisso mais longo), não se funde com `monthlyCommitted`/o gauge de % da renda — dívida (decrescente, com fim) e recorrência (piso de gasto contínuo) são conceitos diferentes. Sem régua de cor por ora; oculta quando o valor é 0.
