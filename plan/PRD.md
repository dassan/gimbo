...
* **F-24:** Patrimônio Líquido (Net Worth) — página dedicada na navbar (`/net-worth`) com stat cards, breakdown por conta e gráfico de evolução histórica mensal (AreaChart). Ativos = contas não-CREDIT com `includeInBalance` + valuations de STOCKS/CRYPTO/FOREX/ASSET; passivos = total parcelado em aberto de cada conta CREDIT + `getLoanLiability` de cada conta LOAN + parcelamentos abertos em conta comum, todos via `getDebtBreakdown` (mesmo motor de `/health`, M-85/M-86 — antes, cartão usava só a fatura atual via `getTotalCreditLiability`). Toggle `netWorthIncludeHidden` controla a inclusão de contas com `includeInBalance=false`. Ver épico `plan/BACKLOG.md` (NW-01 a NW-08, M-85, M-86) e `plan/NET_WORTH.md`.
